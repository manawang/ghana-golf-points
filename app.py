    ...
    if st.button("🚀 开始计算积分", type="primary", use_container_width=True):
        with st.spinner("正在计算积分..."):
            ...
            display_df = df_results[[
                'net_rank', 'name', 'gross_score', 'net_score', 
                'base_points', 'bonus_points', 'total_points', 'is_gross_champion'
            ]].rename(columns={
                'net_rank': '净杆排名',
                'name': '姓名',
                'gross_score': '总杆',
            ...
            # 冠军展示
            champions = [r for r in points_results if r['is_gross_champion']]
            if champions:
